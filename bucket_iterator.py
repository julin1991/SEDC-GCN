...
class BucketIterator(object):
    ...
    def pad_data(self, batch_data):
        ...
        for item in batch_data:
            text_indices, context_indices, aspect_indices, left_indices, polarity ,post_emb,seq_graph,pmi_graph,xuhao = \
                item['text_indices'], item['context_indices'], item['aspect_indices'], item['left_indices'],\
                item['polarity'],item['post_emb'], item['seq_graph'], item['pmi_graph'],item['xuhao']
            ...
            batch_xuhao.append(xuhao)
            dere_num = 2
            redependency_graph = []
            for i in range(dere_num):
                adj = numpy.pad(pmi_graph[i], \
                                ((0, max_len - len(text_indices)), (0, max_len - len(text_indices))), 'constant')
                redependency_graph.append(adj)
            batch_pmi_graph.append(redependency_graph)
            batch_seq_graph.append(numpy.pad(seq_graph, \
                ((0,max_len-len(text_indices)),(0,max_len-len(text_indices))), 'constant'))
        ...
        return { \
                ...
                'seq_graph': torch.tensor(batch_seq_graph), \
                'pmi_graph': torch.tensor(batch_pmi_graph), \
                'xuhao': batch_xuhao
